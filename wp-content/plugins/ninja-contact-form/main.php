...
function ncf_add_subscriber($email, $token, $list,  $first_name = NULL, $last_name = NULL, $phone = NULL, $city = NULL)
{
    $response = array();
    ...
    if (isset($email) && filter_var($email, FILTER_VALIDATE_EMAIL)) {
        try {
            ...
            try {
                $response = $mc->lists->subscribe($list, array('email' => $email), array('FNAME' => $first_name, 'LNAME' => $last_name));
                $ml_last_error = '';
            } catch (LA_Mailchimp_Error $e) {
                $response = false;
