...
//			  $('<div id="nks-fixed-bg">').appendTo($body);
			...
			function init() {
				...
				$body.delegate('.ncf_trigger_element' + (opts.togglers ? ',' + opts.togglers : ''), 'click tap', function(){
					if (isExposed) hideSidebar($(this))
					else showSidebar($(this));
					return false;
				})
        ...
        $( document ).ajaxComplete(function() {
          setTimeout(function(){
            if(!$body.is('.ncf_on')) {
              $body.addClass(bodyClasses + (isExposed ? ' nks_cc_exposed' : ''));
            }
          }, 0);
        });
...
//			    $bodybg.height(_h);
        ...
        $scrollable.scrollTop(0);
				...
				if ($.fn.revpause) $('.rev_slider').revpause();
