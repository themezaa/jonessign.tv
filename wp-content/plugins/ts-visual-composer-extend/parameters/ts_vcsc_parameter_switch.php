    ...
    if (!class_exists('TS_Parameter_Switch')) {
        class TS_Parameter_Switch {
            ...
            function switch_button_settings_field($settings, $value) {
				...
				$layout			= isset($settings['layout'])        ? $settings['layout']       : 'switchblock'; // switchblock, switchflat
                ...
                $output         = '';
				if ($layout == "switchblock") {
					$output .= '<div class="ts-switch-button ts-composer-switchblock" data-value="' . $value . '" data-render="' . $render . '" data-layout="' . $layout . '" data-width="' . $width . '" data-style="' . $style . '" data-on="' . $on . '" data-off="' . $off . '">';
						...
						$output .= '</div>';
					$output .= '</div>';
				} else if ($layout == "switchflat") {
					$output .= '<div class="ts-switch-button ts-composer-switchflat" data-value="' . $value . '" data-render="' . $render . '" data-layout="' . $layout . '" data-width="' . $width . '" data-style="' . $style . '" data-on="' . $on . '" data-off="' . $off . '">';
						$output .= '<input type="hidden" style="display: none; " class="toggle-input wpb_vc_param_value ' . $param_name . ' ' . $type . '" value="' . $value . '" name="' . $param_name . '"/>';
						$output .= '<input type="checkbox" value="' . ($value == 'true' ? 1 : 0) . '" ' . ($value == 'true' ? 'checked="checked"' : '') . '>';
					$output .= '</div>';
				}
                return $output;
            }            
