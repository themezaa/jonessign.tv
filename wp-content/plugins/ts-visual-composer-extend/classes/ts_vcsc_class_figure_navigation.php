	...
	if (!class_exists('TS_Figure_Navigation')){
		class TS_Figure_Navigation {
			function __construct() {
				...
				if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VCFrontEditMode == "true") {
					if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VisualComposer_LeanMap == "true") {
						add_action('init',                                  array($this, 'TS_VCSC_Add_Figure_Navigation_Lean'), 9999999);
			...
			function TS_VCSC_Figure_Navigation_Item ($atts, $content = null) {
				...
				$output .= '<div class="ts-figure-navigation-item ' . $icon_tooltipclasses . ' ' . ($frontend_edit=="true" ? "frontend" : "") . '" ' . $icon_tooltipcontent . ' style="width: 25%; height: 110px; ' . $figure_background_style . ';" data-active="' . $figure_active . '" data-frontend="' . $frontend_edit . '">';
