	...
	if (!class_exists('TS_Google_Maps_Plus')){
		class TS_Google_Maps_Plus {
			...
			function __construct() {
				...
				if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VCFrontEditMode == "true") {
					if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VisualComposer_LeanMap == "true") {
						add_action('init',                                  array($this, 'TS_VCSC_Add_GoogleMapsPlus_Elements_Lean'), 9999999);
			...
			function TS_VCSC_GoogleMapsPlus_Container ($atts, $content = null) {
				...
				$map_import							= 'data-import-markerjson="' . $import_json_marker . '" data-import-markerpath="' . $import_path_marker . '"';
				
				// Compile Language Settings
				$map_language						= 'data-string-otherlink="' . $string_other_link . '" ';			
				...
				if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VCFrontEditMode == "false") {
					$output .= '<div id="' . $map_id . '" class="' . $css_class . '" data-random="' . $map_random . '" ' . $map_fullwidth . ' style="margin-top: ' . $margin_top . 'px; margin-bottom: ' . $margin_bottom . 'px;">';
						...
						$output .= '<div id="ts-advanced-google-map-wrapper-' . $map_random . '" class="ts-advanced-google-map-wrapper ' . ($googlemap_tiles == 'true' ? 'ts-advanced-google-map-tiled' : '') . '" style="height: ' . $googlemap_height . 'px;" ' . $map_data . ' ' . $map_controls . ' ' . $map_layers . ' ' . $map_draggable . ' ' . $map_import . ' ' . $map_snazzystyle . ' ' . $map_language . '></div>';
					$output .= '</div>';
				} else {
			...
			function TS_VCSC_GoogleMapsPlus_Single ($atts, $content = null) {
				...
				$marker_data						= 'data-id="map-marker-single-' . $map_random . '" data-processed="false" data-group="" data-streetview="' . $marker_streetview . '" data-draggable="' . $marker_draggable . '" data-title="' . $marker_title . '" data-icon="' . $marker_icon . '" data-animation-allow="' . $marker_animation . '" data-animation-type="' . $marker_entry . '" data-latitude="' . ($marker_position == 'coordinates' ? $marker_latitude : '') . '" data-longitude="' . ($marker_position == 'coordinates' ? $marker_longitude : '') . '" data-address="' . ($marker_position == 'address' ? $marker_address : '') . '" data-draggable="false" data-popup="' . $marker_popup . '"';
				...
				// Compile Main Data
				$map_data							= 'data-initialized="false" data-mapheight="' . $googlemap_height . '" data-singleinfo="' . $googlemap_singleinfo . '" data-listeners="false" data-delay="' . $googlemap_delay . '" data-mapsearch="' . $controls_search . '" data-activate="' . $googlemap_mobile . '" data-metric="' . $googlemap_metric . '" data-centertype="' . $center_type . '" data-latitude="' . ($center_type == 'coordinates' ? $center_latitude : '') . '" data-longitude="' . ($center_type == 'coordinates' ? $center_longitude : '') . '" data-address="' . ($center_type == 'address' ? $center_address : '') . '" data-zoom="' . $googlemap_zoom . '" data-maptype="' . $googlemap_type . '" data-mapstyle="' . $googlemap_style . '" data-openlayers="' . $googlemap_openlayers . '" data-mapclusters="false" data-mapresize="' . $googlemap_resize . '"';
				$map_controls						= 'data-controls-home="' . $controls_home . '" data-controls-bounds="' . $controls_bounds . '" data-controls-types="' . $controls_types . '" data-controls-pan="' . $controls_pan . '" data-controls-zoomer="' . $controls_zoomer . '" data-controls-wheel="' . $controls_wheel . '" data-controls-styler="' . $controls_styler . '" data-controls-groups="false" data-controls-select="false" data-controls-street="' . $controls_street . '" data-controls-scaler="' . $controls_scaler . '"';
				$map_draggable						= 'data-draggable-allow="' . $draggable_allow . '" data-draggable-width="' . $draggable_width . '"';
				$map_layers							= 'data-layers-biking="' . $layers_biking . '" data-layers-traffic="' . $layers_traffic . '" data-layers-transit="' . $layers_transit . '"';
				$map_fullwidth						= 'data-fullwidth="' . $googlemap_full . '" data-break-parents="' . $googlemap_breaks . '"';
				if ($googlemap_style != "style_snazzyimport") {
				...
				if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VCFrontEditMode == "false") {
					$output .= '<div id="' . $map_id . '" class="' . $css_class . '" data-random="' . $map_random . '" ' . $map_fullwidth . ' style="margin-top: ' . $margin_top . 'px; margin-bottom: ' . $margin_bottom . 'px;">';
						...
						$output .= '<div id="ts-advanced-google-map-directions-' . $map_random . '" class="ts-advanced-google-map-directions" style="display: none;"></div>';
						$output .= '<div id="ts-advanced-google-map-streetview-' . $map_random . '" class="ts-advanced-google-map-streetview" style="display: none; height: ' . $googlemap_street . 'px;"></div>';
						$output .= '<div id="ts-advanced-google-map-wrapper-' . $map_random . '" class="ts-advanced-google-map-wrapper" style="height: ' . $googlemap_height . 'px;" ' . $map_data . ' ' . $map_controls . ' ' . $map_layers . ' ' . $map_draggable . ' ' . $map_import . ' ' . $map_snazzystyle . ' ' . $map_language . '></div>';
					$output .= '</div>';
				} else {
				...
				$VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VisualComposer_Element = array(
					...
					"params"                    	=> array(
						...
						array(
							...
							"value"                 => "",
							"admin_label"           => true,
							"description"	        => __( "Optionally, please provide the name of a group (single) this marker belongs to.", "ts_visual_composer_extend"),
						),
						array(
							"type"			        => "dropdown",
							"class"			        => "",
				...
				$VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VisualComposer_Element = array(
					...
					"params"                    	=> array(
						...
						array(
							...
							"value"                 => "",
							"admin_label"           => true,
							"description"	        => __( "Optionally, please provide the name of a group (single) this overlay belongs to.", "ts_visual_composer_extend"),
						),	
						// Circle Settings
						array(
							"type"              	=> "seperator",
