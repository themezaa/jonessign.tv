	...
	var Mapplic = function() {
		...
		function Sidebar() {
			...
			this.addLocation = function(data) {
				...
				var link = $('<a></a>').attr('href', '#' + data.id).appendTo(item).click(function(e) {
					e.preventDefault();
		...
		function PinDrag() {
			...
			this.update = function(location) {
				...
				if (location.description) {
					...
					else tinymce.activeEditor.setContent(location.description);
				}
						...
						case 'svg':							
							$('<div></div>').addClass('mapplic-map-image').load(source, function() {
								...
								$(self.o.selector, this).on('click touchend', function() {
									self.locationid.text($(this).attr('id'));
								});
							}).appendTo(layer);
							break;
