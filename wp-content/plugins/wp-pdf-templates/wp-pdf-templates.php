 ...
 */
set_pdf_print_support(array('post', 'page', 'portfolio'));
...
function _print_pdf($html) {
  ...
  if (isset($wp_query->query_vars['pdf'])) {
    // convert to PDF
    // building the filename as yyyy_mm_dd_jonessign_project_project_title_hour_minute.pdf on download
    $downloaded_date = date( 'Y_m_d');
    $filename_append = date('H_i');  // added the hour and minute before the filetype as I am testing and there will be more than one download a day, but I would think that this could normally be gotten rid of
    $filename = $downloaded_date . '_jonessign_project_' . strtolower(str_replace(' ','_',get_the_title() ) ) . '_' . $filename_append;
