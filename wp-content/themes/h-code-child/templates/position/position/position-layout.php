...
elseif(is_author()):
 ...
 $title = 'Open Positions at Jones Sign Company';
...
<section class="parent-section <?php echo $section_class.' '.$hcode_header_menu_type; ?>">
    <div class="<?php echo $class_main_section; ?>">
            <div class="row">
                    <?php get_template_part('templates/position/position-archive-left'); ?>
                <?php
                    get_template_part('templates/position/position-page-content/'.$hcode_layout_settings.'/content',get_post_format());
                ?>
                    <?php get_template_part('templates/position/position-archive-right'); ?>
            </div>
    </div>
</section>
