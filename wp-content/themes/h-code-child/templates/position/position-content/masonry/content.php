...
if( have_posts() ):
    echo '<div id="positions-layout" class="blog-masonry blog-masonry-2col">';
        while ( have_posts() ) : the_post();
            ...
            echo '<div id="single-position" class="'.$class.' blog-listing position-listing">';
                ...
                if(!empty($blog_image)){
                    echo '<div class="position-post">';
                        get_template_part('loop/loop','image');
                }
                elseif(!empty($blog_gallery)){
                    echo '<div class="position-post position-post-gallery">';
                        get_template_part('loop/loop','gallery');
                }
                elseif(!empty($blog_video)){
                    echo '<div class="position-post position-post-video">';
                ...
                echo '<div id="position-post" class="position-archive position-post">';
                        $thumb = wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()), 'full' );
                        $url = $thumb['0'];
                        echo '<div id="position-image" class="position-archive position-image"><a href="'.get_permalink().'">';
                        if ( has_post_thumbnail() ) {
                            echo get_the_post_thumbnail( get_the_ID(), 'full',$img_attr );
                        }
                        else {
                                echo '<img src="' . HCODE_THEME_ASSETS_URI . '/images/jones_alt_800x600.png" width="800" height="600" alt=""/>';
                        }
                    echo '</a></div><!-- end div#position-image -->';
                }
                    echo '<div id="position-details" class="position-archive position-details">';
                        echo '<div id="position-posted-on" class="position-archive position-date pull-right"> POSTED ON: '.$show_date.'</div><!-- end div#position-posted-on -->';
                        echo '<div id="position-title" class="position-archive position-title"><a href="'.get_permalink().'">'.get_the_title().'</a></div><!-- end div#position-title -->';
                    echo '</div><!-- end div#position-details -->';
                echo '</div><!-- end div#position-post -->';
            echo '</div><!-- end div#single-position -->';
        endwhile;
    echo '</div><!-- end div#positions-layout -->';
