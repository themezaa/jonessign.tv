				...
				<address style="font: normal 10px/15px Arial, sans-serif; color:<?php echo $options['colors']['primary']; ?>;">
					<?php
						if($address != ''){
							...
							$output .= $branchname;
							$output .= '</h3>';
							$output .= '<h3 style="margin-top:-1em">';
							$output .= $branchphone;
							$output .= '</h3>';
							$output .= '<h3 style="margin-top:-1em">';
							$output .= $address;
...
<body>
	...
	<form id="email-signature-form" action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" target="_blank" autocomplete="off">
							...
							<?php foreach( $options['address_list'] as $address ) : ?>
								<option value="<?php echo $address['abbreviation']; ?>">
									<?php
										...
										$branchphone = $address['branchphone'];
										echo $address['branchname'];
									?>
								</option>
							<?php endforeach; ?>
						</select>
						<input name="mailing-address" type="hidden" value="" />
						<input name="branchphone" type="hidden" value="<?php echo $branchphone; ?>" />
						<input name="branchname" type="hidden" value="<?php echo $branchname; ?>" />
					</td>
			</tr>
			<?php endif; ?>
		...
		<!-- BEGIN EMAIL SIGNATURE PREVIEW -->
		<div class="signature-preview">
			<table style="width:100%;">

			<!-- BEGIN NAME, POSITION, DEPARTMENT -->
				<tr>
					<td style="border-bottom: 1px solid <?php echo $options['colors']['tertiary']; ?>; padding-bottom: 14px;">
				...
				<tr>
					<td>
						<a  style="width:125px; margin:16px 0 8px; display:block;"
							...
							target="_blank"
							title="<?php echo $options['company_name']; ?>">
							<img src="<?php echo $options['logo_url']; ?>"
									alt="<?php echo $options['company_name']; ?>"
									width="125px" />
						</a>
					</td>
				</tr>
				...
				<!-- BEGIN PREVIEW OF SMALLER INFORMATION ROWS -->
				<tr>
					<td colspan="2">
						<address id="email-address" style="font: normal 10px/15px Arial, sans-serif; color:<?php echo $options['colors']['primary']; ?>;">
						...
						<?php if( !$options['hide_address_field'] ) : ?>
							<!-- begin branchname -->
							<address id="branchname" style="font: normal 10px/15px Arial, sans-serif; color:<?php echo $options['colors']['primary']; ?>">
							<?php echo $branchname; ?>
							</address>
							<!-- end branchname -->

							<!-- begin branchphone -->
							<address id="branchphone" style="font: normal 10px/15px Arial, sans-serif; color:<?php echo $options['colors']['primary']; ?>">
							<?php echo $branchphone; ?>
							</address>
							<!-- end branchphone -->

							<address id="address" style="font: normal 10px/15px Arial, sans-serif; color:<?php echo $options['colors']['primary']; ?>">
							<?php echo $options['address_list'][0]['address']; ?>
							</address>
						<?php endif; ?>
						<!-- end preview-->
					</td>
				</tr>
				<!-- end Preview of smaller information rows-->
...
<script>
	$(function(){
		...
		$('#email-signature-form').submit(function(){
			...
			//
			var branchname = $('#email-signature-form select[name="branchname"]').val();
			var branchphone = $('#email-signature-form select[name="branchphone"]').val();
		...
		$('input[name="full-name"], input[name="position"], input[name="address"],input[name="branchname"], input[name="email"], input[name="primary-number"], input[name="secondary-number"]').keyup(function(){
			...
			<?php if( !$options['hide_address_field'] ) : ?>
				<?php foreach( $options['address_list'] as $address ) : ?>
					...
					if(location == '<?php echo $address['abbreviation']; ?>'){
						...
						branchname = '<?php echo $address['branchname']; ?>';
